...
export default function Detailplaylist({ auth, playNavbar, rilisPlaylist, DetailTutorial, Detailplaylist }) {
...
Detail.author.name.toLowerCase().includes(searchTerm.toLowerCase())
);
        ...
        <div className="glass m-auto card w-32 rounded-full mt-4">
            <div className="avatar online  ">
                <div className="w-32 rounded-full">
                    <img src={`/storage/${Detailplaylist.image}`} />
                </div>
            </div>
        </div>
        <div className="card w-full glass">
            <div className="card-body">
                <h2 className="font-bold text-xl text-center">Playlist {Detailplaylist.name}</h2>
                <p className="text-gray-700 text-sm"><i class="fa-solid fa-circle-play"></i> Jumlah Materi :
                    {Detailplaylist.tutorials_count} </p>
                <p className="text-gray-700 text-sm"><i className="mr-1 fa-solid fa-calendar-week fa-bounce"></i>
                    Updated Materi : {Detailplaylist.updated_at.split("T")[0]} </p>
                <div className="flex items-center w-80">
                    <div className="relative w-full">
                        ...
                        placeholder={`Search title in ${Detailplaylist.name}`} required />
                    </div>
                </div>
            </div>
        </div>
    ...
    <section className="bg-white  dark:bg-gray-900">
        ...
        <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
                ...
                <>
                    ...
                    <div className='shadow-xl p-2 rounded-lg border border-primary hover:border-indigo-300' >
                        ...
                        <h3 className="my-2 text-xl font-bold dark:text-white">{Detail.title} <span
                                className="text-sm ">|
                                #{Detailplaylist.name}</span> <span className="btn btn-xs ">{i+1}</span><span
                        ...
                        <a href={`/author/${Detail.author.id_author}`} className="text-gray-500  text-xs flex"><img
                                src={`/storage/${Detail.author.profil}`} alt="Profil"
                                className="w-7 h-7 rounded-full mr-2" /> <span
                                className="mt-1 ">{Detail.author.name}</span></a>
            ...
            <div className="text-center">
                <p className="text-center flexfont-bold text-gray-500 p-3">{filteredDetailTutor.length == 0 ? (
                    <span>--- Upss !! Judul atau Author {searchTerm} tidak ditemukan ---</span>
                    ) : ''}
                </p>
            </div>
        ...
        <div className="m-8 bg-gray-200  rounded-b-3xl ">
            <div className="py-2 px-5 rounded-lg mb-5 ">
                ...
                <h3 className="mb-2 text-sm lg:text-xl text-center font-bold dark:text-white">Tidak ada materi dalam
                    playlist {Detailplaylist.name}</h3>
            </div>
        </div>
        )}
        <br />
